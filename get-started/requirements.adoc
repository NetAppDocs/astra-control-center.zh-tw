...
| Astra Trident  a| 
Astra Trident 210.1或更新版本已安裝並設定Astra Trident 22.04或更新版本、以進行SnapMirror型應用程式複寫
...
* Astra Control Center必須部署在第三個故障網域或次要站台、才能實現無縫的災難恢復。
...
* 來源ONTAP 叢集和目的地叢集上必須同時存在Trident版本22.04或更新版本。
...
----
+
請參閱 https://docs.netapp.com/us-en/trident/trident-use/backends.html["設定後端"^] 以取得更多資訊。

* 當您在ONTAP Astra Control Center中新增一個功能不全的儲存後端時、您用來在Astra Control Center和儲存後端之間進行通訊的使用者、必須在ONTAP 兩個叢集上的「http」和「ontapi」內啟用使用者登入存取方法ONTAP 。請參閱 https://docs.netapp.com/us-en/ontap-sm-classic/online-help-96-97/concept_cluster_user_accounts.html#users-list["管理使用者帳戶"^] 以取得更多資訊。
...
瞭解操作方法 link:../use/replication.html["使用SnapMirror技術將應用程式複寫到遠端系統"]。
