...
現在您已確認已符合先決條件、您已經準備好了  cluster,新增叢集。
...
.您需要的是 #8217 ；需要的是什麼
* 新增叢集之前、請先檢閱並執行必要的  your environment for cluster management using Astra Control,必要工作。
...
IMPORTANT: 如果您自行建立 `kubeconfig` 檔案中、您應該只定義*一個*內容元素。請參閱 https://kubernetes.io/docs/concepts/configuration/organize-cluster-access-kubeconfig/["Kubernetes文件"^] 以取得有關建立的資訊 `kubeconfig` 檔案：如果您使用為有限的叢集角色建立了Kbeconfig  a limited cluster role kubeconfig,上述程序請務必在本步驟中上傳或貼上該KECBEConnfig。
