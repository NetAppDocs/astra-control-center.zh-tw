...
您也可以從Astra Control Center Storage後端頁面監控Astra Data Store儲存後端處理量、IOPS和容量。請參閱 link:../use/manage-backend.html["管理儲存後端"]。



== 新增Proxy伺服器以連線至Cloud Insight或NetApp支援網站
...
此外、如果您在Astra Control上管理Astra Data Store叢集（連線Cloud Insights 至各處）、則會在Astra Data Store上為每個Astra Data Store叢集建立一個擷取單元Pod、並將指標從Astra Data Store傳送至配對Cloud Insights 的資料系統。每個Pod需要8 GB RAM和2個CPU核心。
