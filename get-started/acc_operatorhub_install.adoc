...
[role="lead"]
如果您使用Red Hat OpenShift、可以使用Red Hat認證的操作員來安裝Astra Control Center。請使用此程序從安裝Astra Control Center https://["Red Hat生態系統目錄"^] 或使用Red Hat OpenShift Container Platform。
...
** 從Red Hat生態系統目錄：
+
... 選擇NetApp Astra Control Center https://["營運者"^]。
... 選擇*部署和使用*。
...
.. 輸入 Astra Control Center 版本、例如 23.04.0-22 。
.. 輸入帳戶名稱、電子郵件地址和管理員姓氏。
