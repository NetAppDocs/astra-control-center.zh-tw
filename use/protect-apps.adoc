...
[role="lead"]
使用自動保護原則或以臨機操作的方式、擷取快照與備份資料、以保護所有應用程式。您可以使用Astra Control Center UI或 https://docs.netapp.com/us-en/astra-automation/index.html["Astra Control API"^] 保護應用程式。
...
NOTE: 如果您需要取消執行中的備份、請依照中的指示操作  backups。若要刪除備份、請等到備份完成後再依照中的指示進行  backups。
...
NOTE: 如果您需要取消執行中的備份、請依照中的指示操作  backups。若要刪除備份、請等到備份完成後再使用這些指示。
