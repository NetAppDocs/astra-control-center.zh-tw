...
如需安裝指示、請參閱 https://["在OpenShift Container Platform的AWS上安裝叢集"^]。
...
請遵循AWS文件完成下列步驟。請參閱 https://["AWS安裝文件"^]。
...
* https://["開始使用Cloud Volumes ONTAP AWS的功能"^]。
* https://["使用BlueXP在AWS中建立連接器"^]
...
請遵循BlueXP文件完成下列步驟。請參閱 https://["從GCP開始使用Cloud Volumes ONTAP"^]。
...
.步驟
. 將您的認證資料新增至BlueXP。請參閱 https://["新增GCP帳戶"^]。
. 新增GCP的連接器。
+
.. 選擇「GCP」作為供應商。
.. 輸入GCP認證。請參閱 https://["從BlueXP在GCP中建立連接器"^]。
.. 確認連接器正在執行、並切換至該連接器。
...
請依照Azure文件完成下列步驟。請參閱 https://["在Azure上安裝OpenShift叢集"^]。
...
請遵循BlueXP文件完成下列步驟。請參閱 https://["Azure中的BlueXP入門指南"^]。
...
.步驟
. 將您的認證資料新增至BlueXP。
. 新增Azure連接器。請參閱 https://["BlueXP原則"^]。
+
.. 選擇* Azure *作為供應商。
.. 輸入Azure認證資料、包括應用程式ID、用戶端機密和目錄（租戶）ID。
+
請參閱 https://["從BlueXPr在Azure中建立連接器"^]。
